#################################################
# HelloID-Conn-Prov-Target-HR2Day-Delete
# PowerShell V2
#################################################
...
try {
    ...
    # To-Do: Add logic to get user details from HR2Day based on the account reference
    $correlatedAccount = @{
    ...
    #So if user is found, we can just assume the action is a delete.
    ...
    switch ($action) {
        'DeleteAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                Write-Information "[DryRun] Delete HR2Day account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete HR2Day value. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditMessage = "Could not delete HR2Day value. Error: $($ex.Exception.Message)"
